...
const Unit = () => {
  ...
  const confirmBulkDelete = async () => {
    ...
    try {
      ...
      refetch();
      toast.success(`Successfully deleted ${bulkDeleteIds.length} unit${bulkDeleteIds.length > 1 ? 's' : ''}`);
    } catch (err) {
  ...
  if (error)
    return (
      <div className="p-6 bg-red-50 border border-red-200 rounded-lg">
        <p className="text-red-600 font-medium">Error loading units</p>
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      {bulkDeleteIds && (
        <DeleteDialog
          ...
          message={`Are you sure you want to delete ${bulkDeleteIds.length} unit${bulkDeleteIds.length > 1 ? 's' : ''}? This action cannot be undone.`}
